...
| G | retry_command 絞り込み | `retry_command` ターゲットを失敗ファイルに限定、`--only-failed` で直前 run の失敗組み合わせのみ再実行 |

パートA〜G完了後、タグ付け前にリリース前整備を実施する。コード構造リファクタ・API設計改善（`ToolTargets` dataclass・`--from-run`追加）・運用改善（run_idテキスト表示・mkdocsアンカー修正）の三系統をまとめて対応する。
...
- [詳細参照サブコマンド](詳細参照サブコマンド.md)
- [MCP サーバー化](MCPサーバー化.md)
- [retry_command 絞り込み](retry_command絞り込み.md)
- [リリース前整備](リリース前整備.md)
...
- `retry_command`のターゲットを当該ツールで失敗したファイルのみに絞り込み、JSONL出力を軽量化する ([retry_command 絞り込み](retry_command絞り込み.md))
- `--only-failed`: 直前runの実行アーカイブから失敗ツールと失敗ファイルを抽出し、ツール別に失敗組み合わせのみを再実行するCLIオプション
- `--from-run`: `--only-failed`の参照対象runを明示指定するCLIオプション（前方一致・`latest`エイリアス対応）([リリース前整備](リリース前整備.md))
