...
- `python = false`: Python系ツールを一括無効化する。対象ツールの一覧は[docs/configuration.md](configuration.md)の「Python系ツールの一括無効化」を参照。
...
日本語ドキュメントでは行長制限が実用的でないため、`line-length` チェックのみ無効化している。
...
`mise.toml`（`uvx pyfltr`経由で実行する構成）:
...
mise.toml / .pre-commit-config.yaml / CI構成はRustプロジェクトと同様。
