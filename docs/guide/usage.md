...
- `--output-format`: `text`（行形式 `key: value`）・`json`（単発dict）・`jsonl`
 （`kind: "meta"` / `"tool"` / `"diagnostic"` / `"output"` 種別の1行1レコード）
...
| 主な用途 | pre-commitフック等 | ローカルで全チェック | LLMエージェント呼び出し | CI・コミット前 |
...
- `header`: 先頭1行。実行環境情報
 （`version` / `run_id` / `python` / `platform` / `cwd` / `commands` / `files` / `schema_hints`）
...
{"kind":"diagnostic","tool":"mypy","file":"src/a.py","messages":[{"line":42,"col":5,"msg":"Incompatible return value type"}]}
...
`tool`レコードの`rc`は`returncode is not None`のときのみ含まれる（`skipped`では省略）。

`diagnostic`レコードは`(tool, file)`単位で集約される。個別指摘は`messages[]`配列に格納され、
...
`tool`レコードは`status == "failed"`かつ`diagnostics == 0`のときに限り、
...
- `hint-urls`: 当該ツールで出た各ruleのドキュメントURL辞書（キーはrule ID、値はURL）。
...
- `jsonl-diagnostic-limit`: 1ツールあたりの出力上限（既定 `0` = 無制限）。
    `(tool, file)`集約後の行数ではなく`messages[]`の合計件数で判定する
...
- `header.schema_hints`: JSONLの各フィールドの意味を短い英文で補足する辞書。
  毎runに同梱されるため、LLM側で事前知識がなくてもJSONLを解釈できる
- `summary.guidance`: `failed > 0`のときだけ付与される英語の配列。
  `tool.retry_command`の参照、`pyfltr run-for-agent --only-failed`の活用、`diagnostic.fix`値の解釈、
