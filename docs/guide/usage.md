...
linterのautofix機能を呼び出す専用モード。textlint / markdownlint / ruff-checkなど、autofix機能を持つlinterに対して内部で`{command}-fix-args`を付与して実行する。手動実行専用。

formatterは本サブコマンドの対象外とする。`black`や`prettier`などのformatterは通常実行（`pyfltr run`など）で常に書き込みモードで動作するため、`pyfltr fix`で重複して走らせる必要は無い。

fixモードの対象は以下の条件を満たすコマンドとなる。

- 有効化されている
- `{command}-fix-args` が定義されている（ビルトインではtextlint / markdownlint / ruff-check / eslint / biome / cargo-clippyが既定で対応）
...
- linterのfix実行後、ファイル内容に変化があれば `formatted`、変化が無ければ `succeeded`、終了コードが0以外なら `failed` となる
...
従来の`--fix`オプションは非推奨となった。指定した場合は警告が出力される。代わりに`pyfltr fix`サブコマンドを使用すること。
...
設定ファイルの雛形を標準出力に書き出す。`[tool.pyfltr]`セクションに貼り付けて利用する。このサブコマンドは他のオプションやターゲット指定を受け付けず、設定出力だけを行う。
...
pre-commit frameworkから呼び出したい場合は、`.pre-commit-config.yaml`の`repo: local`でlocal hookとして`uv run pyfltr`または`uvx pyfltr`を呼び出す。
...
- pre-commit hookで自動修正まで走らせたい場合は、fast実行に加えて`pyfltr fix`を走らせるlocal hookを別途並べる
- `pyfltr fix`は`{command}-fix-args`が定義されたlinter（`cargo-clippy` / `ruff-check` / `textlint`等）のみを対象とする
- formatter（`black` / `prettier` / `cargo-fmt` / `dotnet-format`等）は通常実行で常時書き込みモードで動作するため、`pyfltr fix`で追加実行する必要は無い
