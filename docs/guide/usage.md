...
`run` / `fast` サブコマンドは、formatter段の前にfixステージを内蔵する。

fixステージでは`{command}-fix-args`が定義された有効なlinterを`--fix`付きで順次実行する。対象ツールは`ruff-check` / `textlint` / `markdownlint` / `eslint` / `biome` / `cargo-clippy`など。`ruff check --fix` → `ruff format` → `ruff check`のような2段階処理をpyfltrのパイプライン全体で実現する位置づけ。

fixステージの結果は最終サマリーに含めない。自動修正の痕跡は後続のformatter段やlinter段で間接的に観測される（formatterの`formatted`判定、linterの残存違反など）。

- `run` / `fast` で自動的に有効。`ci`は副作用を嫌うサブコマンドのため無効
- `--no-fix`で明示的に抑止可能
...
`--output-format`で出力形式を切り替えられる。既定は`text`（従来の人間向け出力）で、CLIオプションのみ対応する（`pyproject.toml`側での既定値指定は廃止済み）。
...
終了コードは従来と同じで、`--exit-zero-even-if-formatted`と組み合わせるとformatterによる修正だけでは`exit 1`にならないようにできる。
...
```shell
pyfltr run --output-format=jsonl --output-file=.claude/lint.jsonl
```
