...
- `header`: 先頭1行。実行環境情報。
  主なフィールド: `version` / `run_id` / `python` / `executable` / `platform` / `cwd` /
  `commands` / `files` / `uv_lock_present` / `uv_available` / `format_source`。
  `commands`は実行対象（有効化された、`--only-failed`適用後の）コマンド名配列。
  `uv_lock_present`はcwdに`uv.lock`が存在するか（真偽値、常時出力）。
  `uv_available`は`uv`バイナリがPATHから利用できるか（真偽値、常時出力）。
  この2値を組み合わせると、Python系ツールがuv経路で実行されたかdirectフォールバックしたかを判別できる。
  `format_source`は出力形式の解決経路ラベルで、値は
「`cli` / `env.PYFLTR_OUTPUT_FORMAT` / `subcommand_default` / `env.AI_AGENT` / `fallback`」の5語彙。
  実行対象にmise経路のツール（`cargo-fmt` / `actionlint`等）が含まれる場合、`mise_active_tools`を付与する。
  フィールドは取得状況（`status`・取得失敗時のみ`detail`・取得成功時のみ`active_keys`）を格納する。
...
```json
{"kind":"header","version":"3.0.0","run_id":"01HXYZ...","python":"3.12.0 ...","executable":"/usr/bin/python3","platform":"linux","cwd":"/work","commands":["textlint","ruff-format"],"files":12,"uv_lock_present":true,"uv_available":true,"format_source":"subcommand_default"}
...
{"kind":"command","command":"textlint","type":"linter","status":"failed","effective_runner":"js-pnpx","runner_source":"default","files":12,"elapsed":0.8,"diagnostics":1,"rc":1,"hints":{"ja-technical-writing/sentence-length":"textlint counts up to the period (。) as one sentence; bullet-line splits still count as one. Split with periods to shorten.","messages[].col":"textlint reports col and end_col as cumulative offsets from the text-node start, not in-line offsets"}}
{"kind":"command","command":"ruff-format","type":"formatter","status":"formatted","effective_runner":"uv","runner_source":"default","files":12,"elapsed":0.3,"diagnostics":0,"rc":1,"hints":{"status.formatted":"formatter rewrote files; rerun is not required because the rewrite itself counts as success"}}
...
- `col`: 列番号。抽出できた場合のみ含まれる。
  textlintは`column`系をテキストノード先頭からの累積位置で返す仕様のため、textlint由来の`col`は累積位置となる
- `end_line`: 違反範囲の終端行。範囲を返すツール（現状textlintのみ）で設定される
- `end_col`: 違反範囲の終端列。範囲を返すツール（現状textlintのみ）で設定される。
  `col`と同様にtextlintでは累積位置で出力される
...
- `effective_runner`: 実際にツール起動に使われたrunner種別。
  値は`direct` / `mise` / `uv` / `js-pnpx` / `js-pnpm` / `js-npm` / `js-npx` / `js-yarn` /
  `js-direct`のいずれか。
  ネイティブバイナリ系の`bin-runner`設定（既定`mise`、`direct`に切替可能）はその設定値そのものへ畳み込まれる。
  JS系の`js-runner`設定（既定`pnpx`、`pnpm`/`npm`/`npx`/`yarn`/`direct`に切替可能）は`js-`プレフィクス付きで反映される。
  `resolution_failed`時など起動経路が確定しないケースでは省略される
- `runner_source`: runner決定の経緯。
  値は`explicit`（`{command}-runner`明示）/ `default`（既定値）/ `path-override`（`{command}-path`明示）のいずれか。
  `effective_runner`と同じく省略条件が適用される
...
- `hints`: ruleごとの修正ヒント短文の辞書（キーはrule ID、値はヒント文字列）。
    ...
    textlintコマンドの場合は`messages[].col`キーで`col`/`end_col`フィールドが
