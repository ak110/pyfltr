...
```shell
pyfltr agent [files and/or directories ...]
```

`run`と同じ動作をLLMエージェント向けJSONL出力（`--output-format=jsonl`）で実行するエイリアス。
`pyfltr run --output-format=jsonl`と等価で、Claude Codeなどのエージェントから呼び出す際に短く書ける。

出力形式の詳細は[jsonl形式の使い方](#jsonl形式の使い方)を参照。
...
`fast` / `run` / `agent` サブコマンドは、formatter段の前にfixステージを内蔵する。
...
2. 問題があるツール/ファイルだけ個別に再実行する
    ...
    ```shell
    pyfltr agent --commands=mypy path/to/file.py | tail -30
    ```
