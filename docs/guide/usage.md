...
`run`と同じ動作をコーディングエージェント向けJSONL出力（`--output-format=jsonl`）で実行するエイリアス。
`pyfltr run --output-format=jsonl`と等価で、コーディングエージェントから呼び出す際に用いる。
...
{"kind":"summary","total":2,"commands_summary":{"no_issues":{"succeeded":0,"formatted":1,"skipped":0},"needs_action":{"failed":1,"resolution_failed":0}},"diagnostics":1,"exit":1}
```
