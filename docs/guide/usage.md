...
- `-j N` / `--jobs N`: linters/testersの最大並列数を指定（既定: 8、`pyproject.toml`でも設定可能）。モノレポ検出時は同一ツールのサブプロジェクト実行を同時に開始する件数の算出にも使われる
...
- ツールON/OFFの両方向: 親（起点）で無効・子で有効なら子でのみ実行し、親で有効・子で無効なら子サブプロジェクトはスキップする。
