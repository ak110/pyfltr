...
- uv経由実行（既定`{command}-runner = "uv"`）。
  対象はPython系ツール（mypy / pylint / pyright / ty / pytest / ruff-format / ruff-check / uv-sort）。
  cwdに`uv.lock`があり`uv`バイナリが利用可能な場合は`uv run --frozen <bin>`でプロジェクトのvenv経由で起動する。
  どちらかの条件が満たされない場合は`shutil.which`で本体依存に同梱されたバイナリを直接呼ぶ（directフォールバック）
...
`{command}-runner`の許容値は次の5種。

- `"uv"`: cwdの`uv.lock`検出時は`uv run --frozen <bin>`経由でプロジェクトのvenvにあるツールを呼ぶ。
  `uv.lock`不在またはuv不在時はdirectフォールバック
