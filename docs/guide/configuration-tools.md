...
- python-runner経由（既定`{command}-runner = "python-runner"`）。
  対象はPython系ツール（mypy / pylint / pyright / ty / pytest / ruff-format / ruff-check / uv-sort）に加え、
  semgrep / sqlfluffも該当する。
...
対象はmypy / pylint / pyright / ty / pytest / ruff-format / ruff-check / uv-sortのPython系ツールに加え、
semgrep / sqlfluffも該当する。
...
# lychee-args = ["--format", "json", "--no-progress", "--offline"]
```
