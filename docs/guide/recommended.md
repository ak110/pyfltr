...
- `python = true`: Python系ツールのゲートを開ける。推奨ツール（ruff-format / ruff-check / mypy /
  ...
  依存は`uv run --with="pyfltr[python]" pyfltr ...`等で都度取り込むのが推奨。
  dev依存に固定したい場合は`uv add --dev 'pyfltr[python]'`（pip環境では`pip install 'pyfltr[python]'`）を使う。
...
推奨のwith方式では`pyproject.toml`にpyfltrを記述する必要はなく、`[tool.pyfltr]`セクションのみで完結する。
...
- `--with="pyfltr[python]"`: 毎回最新のpyfltrを解決して実行する（uvがキャッシュするため2回目以降は実用速度）。
  dev依存に`pyfltr[python]`を加えている場合は`entry: uv run pyfltr fast`に置き換えてもよい。
...
Pythonプロジェクトでは`uv run --with="pyfltr[python]" pyfltr`を使い、
非Pythonプロジェクトでは`uvx pyfltr`を使う。
pre-commitはpyfltrの依存に含まれるため、`uv run pre-commit`や`uvx pre-commit`で利用可能になる。

### Makefile

Pythonプロジェクト向けの例。`UV_FROZEN`でlockfileを尊重する。
推奨はwith方式で、毎回最新のpyfltrを解決して実行する。
...
setup:
	...
	uv run --with="pyfltr[python]" pre-commit install

# 依存パッケージをアップグレードし全テスト実行
update:
	env --unset UV_FROZEN uv sync --upgrade --all-groups --all-extras
	uv run --with="pyfltr[python]" pre-commit autoupdate
	$(MAKE) test

# フォーマット + 軽量lint（開発時の手動実行用。自動修正あり）
format:
	uv run --with="pyfltr[python]" pyfltr fast

# 全チェック実行（これを通過すればコミット可能）
test:
	uv run --with="pyfltr[python]" pyfltr run

.PHONY: help setup update format test
```

dev依存に`pyfltr[python]`を固定する運用では、`uv sync`後に`uv run pyfltr ...`を呼び出す形に置き換えられる。

```makefile
# dev依存方式の例（pyproject.toml の [dependency-groups] dev に "pyfltr[python]" を追加）
format:
	uv run pyfltr fast
...
言語を問わず利用可能。
Pythonプロジェクトでmiseを使う場合は`uvx pyfltr`を`uv run --with="pyfltr[python]" pyfltr`に読み替える。
  ...
  test:
    ...
    steps:
      ...
      - name: Test with pyfltr
        run: uv run --with="pyfltr[python]" pyfltr ci --output-format=github-annotations
...
- `uv run --with="pyfltr[python]" pyfltr ci ...`: 毎回最新のpyfltrとPython系ツール依存を解決して実行する。
  事前の`uv sync`は不要。dev依存に`pyfltr[python]`を固定する運用では`uv sync --all-extras --all-groups`の後に
...
非uvプロジェクトではイメージ同梱の`pyfltr`をそのまま起動でき、
uv管理プロジェクトでは`uv run --with="pyfltr[python]" pyfltr ...`で常に最新のpyfltrを解決して実行できる。
...
- `uv run --with="pyfltr[python]" pyfltr`: uv管理プロジェクトで毎回最新のpyfltrを解決して実行する。
  dev依存に固定したい場合は`uv sync --all-extras --all-groups` + `uv run pyfltr ...`に置き換える。
  uvを使わないプロジェクトではイメージ同梱の`pyfltr`を直接呼び出せばよい。
...
```yaml
      - name: Test with pyfltr (changed files only)
        run: uv run --with="pyfltr[python]" pyfltr ci --changed-since=origin/main --output-format=github-annotations
```
...
pyfltr:
  ...
  variables:
    ...
    UV_FROZEN: "1"
  script:
    - uv run --with="pyfltr[python]" pyfltr ci --output-format=code-quality --output-file=code-quality-report.json
  artifacts:
    when: always
    reports:
