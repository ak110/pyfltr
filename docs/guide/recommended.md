...
- `uvx pyfltr fast`: 毎回最新のpyfltrを解決して実行する（uvがキャッシュするため2回目以降は実用速度）。
  dev依存に`"pyfltr[python]"`を加えている場合は`entry: uv run --frozen pyfltr fast`に置き換えてもよい。
...
dev依存に`"pyfltr[python]"`を固定する運用では`UV_FROZEN`でlockfileを尊重しつつ、`uv sync`後に`uv run pyfltr ...`を呼び出す形に置き換えられる。
