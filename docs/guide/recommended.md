...
[pre-commit/pre-commit-hooks](https://github.com/pre-commit/pre-commit-hooks)の推奨フックとして`check-ast`（Python構文チェック）と`debug-statements`（デバッグ用`print`/`breakpoint`の検出）を併用することを推奨する。pyfltrのlocal hookと合わせた構成例を以下に示す。
...
```yaml
  - repo: https://github.com/pre-commit/pre-commit-hooks
    ...
    hooks:
      - id: check-ast
      - id: debug-statements
      # 他の推奨フック: check-yaml, check-toml, end-of-file-fixer, trailing-whitespace など
...
pyfltrを呼び出すタスクランナーの設定例。pyfltrをdev依存に含むPythonプロジェクトでは`uv run pyfltr`、含まないプロジェクトでは`uvx pyfltr`を使う。
...
format:
	SKIP=pyfltr uvx pre-commit run --all-files
	-uvx pyfltr fix
	-uvx pyfltr fast
...
- `format`: `pyfltr fix`でlinterのautofix（`cargo-clippy --fix`、`textlint --fix`等）を実行した後、`pyfltr fast`でformatter＋軽量linterを実行する
...
- `line-length`: 日本語ドキュメントでは行長制限が実用的でないため無効化する
- `no-hard-tabs`: Makefileなどのコードブロック内でタブ文字を使うため、コードブロック内のみ許可する

```yaml
$schema: https://raw.githubusercontent.com/DavidAnson/markdownlint-cli2/v0.20.0/schema/markdownlint-cli2-config-schema.json
config:
  line-length: false
  # Makefileのコードブロック内でタブ文字を使うため、コードブロック内のみ許可する
  no-hard-tabs:
    code_blocks: false
```
