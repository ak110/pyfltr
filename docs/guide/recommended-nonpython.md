...
対象の環境には、開発者の手元だけでなく、インストール実体を外部から受け取る実行環境も含める。
ツールを導入済みのコンテナイメージでCIを動かす構成がこれに当たり、
リポジトリの設定を書き換えても、そのイメージが持つ導入済みの版の配置は変わらない。

この構成でCI定義へ工程を足すかどうかは、イメージ側が同じ設定で導入しているかで決まる。
イメージ側が同じ設定で導入している場合は、供給の時点で配置がそろうため何も足さない。
イメージ側がその設定を持たない場合だけ、checkoutの後に`mise install --force <ツール名>`を実行する工程をCI定義へ置く。
`ghcr.io/ak110/pyfltr`イメージは前者に当たり、actionlintを`symlink_bins`付きで導入する。

イメージを配布する側は、導入の時点で同じ設定を適用する。
miseで導入する場合は`mise use --tool-option symlink_bins=true <ツール名>@latest`のように、
設定を適用するツールの直前へ指定を置く。
この指定が後続の何件目のツールまでに及ぶかは定義されていないため、対象のツールだけを渡す呼び出しへ分ける。
...
- oxlintはeslintコアおよび主要eslintプラグイン由来のルールをRust実装で高速に実行する
      - 公式はeslintの置き換えを第一の推奨とし、併用する場合は`oxlint`を先に実行してから
        `eslint`を実行する構成を案内している
      - 両者はルールの多くを共有するため、併用時は`eslint-plugin-oxlint`で
        eslint側の重複ルールを無効化し、同一の指摘が二重に報告される状態を避ける
