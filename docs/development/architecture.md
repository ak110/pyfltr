...
| モジュール | 責務 |
| --- | --- |
| `config.py` | `pyproject.toml`読み込み・`CommandInfo`定義・プリセット・`PYTHON_COMMANDS`一括有効化 |
...
`mypy`/`pylint`/`pyright`/`ty`/`pytest`/`ruff-format`/`ruff-check`/`uv-sort`は既定で無効。
非Pythonプロジェクトで意図しないツール実行が起こることを避けるため、`python = true`または`{command} = true`の明示が必要。
依存も`pyfltr[python]`オプショナルグループに分離する。
