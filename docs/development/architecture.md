...
サブパッケージ構成の詳細はCLAUDE.mdの「モジュール構成」節を参照。

- CLIエントリ: `cli/main.py`（エントリポイント・サブコマンドdispatch）・
  `cli/parser.py`（argparse構築）・
  `cli/pipeline.py`（`run_pipeline()`本体・非TUI実行器）・
  `cli/output_format.py`（出力形式解決・ログ設定）・
  `output/ui.py`（Textual TUI実行器）・
  `state/stage_runner.py`（CLI/TUI共通ヘルパー）・`state/executor.py`（コマンド実行順制御）
- コマンド実行: `command/dispatcher.py`（実行ディスパッチャー）・
  `command/process.py`（subprocess起動とプロセス管理）・
  `command/mise.py`（mise統合・tool解決。`command/runner.py`と`command/dispatcher.py`から参照）・
  `command/env.py`（subprocess環境構築。`command/dispatcher.py`から参照）・
  `command/runner.py`（runner解決とコマンドライン構築）・
  `command/core.py`（実行コンテキスト型と`CommandResult`）・
  `command/targets.py`（対象ファイル選定）・
  `command/snapshot.py`（ファイル変更検知）・
  `command/error_parser.py`（ツール出力解析）・
  `command/glab.py` / `command/precommit.py` / `command/linter_fix.py` / `command/textlint_fix.py`（個別ツール実行）・
  `command/two_step/`（two-step formatter群）・
  `output/rule_urls.py`（ツール別公式ドキュメントURL）・
  `cli/precommit_guidance.py`（`.pre-commit-config.yaml`解釈）・
  `cli/command_info.py`（`pyfltr command-info`サブコマンド）
- 設定とパス処理: `config/config.py`（`pyproject.toml`読み込み・言語カテゴリゲート）・
  `command/builtin.py`（ビルトインツールの`CommandInfo`定義）・`config/presets.py`（プリセット）・
  `paths.py`（パス区切りの統一）・`warnings_.py`（JSONL `warning`レコード生成）
- 出力フォーマット: `output/jsonl.py`（JSONLとsmart truncation）・`output/sarif.py`（SARIF 2.1.0）・
  `output/github_annotations.py`（GitHub Annotation）・`output/code_quality.py`（GitLab CI Code Quality）・
  `output/formatters.py`（`FORMATTERS`レジストリと`RunOutputContext`）・
  `cli/shell_completion.py`（bash/PowerShell補完スクリプト生成）
- 実行アーカイブと再実行支援: `state/archive.py`（書き込み・読み取り・クリーンアップ）・
  `state/cache.py`（ファイルhashベースのスキップキャッシュ）・`state/runs.py`（`list-runs`/`show-run`）・
  `state/retry.py`（`retry_command`生成）・`state/only_failed.py`（`--only-failed`フィルター）
- MCPサーバー: `cli/mcp_server.py`（FastMCPサーバー本体・MCPツール5種・stdio隔離）
...
`output`と`state`は実行結果である`CommandResult`を扱うために`command/core`を参照する。
...
これを避けるための対症療法である。
詳細な判定ロジックと比較キーは`pyfltr/command/env.py`の`_build_subprocess_env`を参照。
...
エントリポイントとdispatchは`cli/main.py`に分離した。
