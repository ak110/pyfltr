...
| モジュール | 責務 |
| --- | --- |
| `config.py` | `pyproject.toml`読み込み・`CommandInfo`定義・プリセット・言語カテゴリ（python / javascript / rust / dotnet）一括有効化 |
...
`python` / `javascript` / `rust` / `dotnet`の各言語カテゴリに属するツールは既定で無効。
対象外プロジェクトで意図しないツール実行が起こることを避けるため、対応する言語カテゴリキー（例: `python = true`）または`{command} = true`の明示が必要。

プリセットは「言語非依存 + ドキュメント系」のみを対象にし、言語別ツールを一切含まない。
