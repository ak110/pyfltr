...
- `pyfltr/cli/`: CLIエントリポイントと各サブコマンドのハンドラー
    ...
    - `cli/mcp_models.py`: MCPツールのPydanticモデル・スキーマ定義
...
- `pyfltr/command/`: コマンド実行コア
    ...
    - `command/two_step/base.py`: `execute_check_write_two_step` / 共通基底ヘルパー（taplo・shfmt用の2段階処理を集約）
    - `command/two_step/ruff.py`: `execute_ruff_format_two_step`本体（ruff専用2段階処理）
    - `command/two_step/prettier.py`: `execute_prettier_two_step`本体（prettier専用2段階処理）
    - `command/two_step/taplo.py`: `execute_taplo_two_step`（`base.py`の`execute_check_write_two_step`への薄いラッパー）
    - `command/two_step/shfmt.py`: `execute_shfmt_two_step`（`base.py`の`execute_check_write_two_step`への薄いラッパー）
