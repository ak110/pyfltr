...
- `pyfltr/command/`: コマンド実行コア
    ...
    `ensure_mise_available` / `ensure_uv_available` / `cwd_has_uv_lock` / `build_invocation_argv`。
    runner解決とコマンドライン構築
...
Python系ツール（ruff-format / ruff-check / mypy / pylint / pyright / ty / uv-sort / pytest）は
`{command}-runner = "uv"`を既定値とする。
`build_commandline()`は、cwdに`uv.lock`が存在しかつ`uv`が利用可能な場合に`["uv", "run", "--frozen", <bin>]`で呼ぶ。
いずれかが満たされない場合は`shutil.which(<bin>)`で本体依存に同梱されたバイナリを
直接呼ぶ（directフォールバック）。
