...
依存パッケージの脆弱性検知はDependabot alertsと定期監査ワークフロー（`audit.yaml`）の2つの手段で行う。
...
`uv lock`・`uv sync`・`uv run`の成功だけでは配布の成立を確認できない。
上書き設定が適用されない状態で依存解決が成立することを
`uvx --exclude-newer "1 day" --from . pyfltr --version`で確認する。
...
確認が失敗した場合は原因を調べる。
通信障害・パッケージ索引の障害・ビルド環境の不備など、版指定以外の原因を解消して再度実行する。
...
軽量な指定で起動し、終了コードが0であることを確認する。
`--locked`は`uv.lock`が変更されないことを表明する指定であり、
ロックファイルへ未反映の変更が残っている場合に当該確認を失敗させる。
`--frozen`は`uv.lock`を更新せずに実行する指定であり、上書きを変更した直後は
旧ロックから同期した環境を観測するため検出の取りこぼしを起こす。
...
制約を変更したのに版が変わらない場合は、まず再解決の範囲を確認する。
他の依存が課す制約を原因と結論づける前に、上記の指定を伴う再実行で確認する。
...
2026-08-02にuv 0.11.7で実行して確認したところ、`--python-platform linux`と`--python-platform windows`は
いずれも`in 150 packages`を返した。
一方`uv tree`は限定する。同じ確認で`--python-platform windows`にのみ`pywin32`・`colorama`が現れ、
...
本節の根拠を更新する際は上記の確認を再実行し、確認した日付と使用したuvのバージョンを併記する。
