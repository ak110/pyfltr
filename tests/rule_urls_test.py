...
def test_build_rule_url_ruff_existing_priority() -> None:
    ...
    url = pyfltr.output.rule_urls.build_rule_url("ruff-check", "F401", existing_url="https://example.com/x")
    assert url == "https://example.com/x"


def test_build_rule_url_ruff_fallback() -> None:
    """ruff は existing_url が無ければテンプレートで補完する。"""
    url = pyfltr.output.rule_urls.build_rule_url("ruff-check", "F401")
...
def test_build_rule_url_pylint_requires_category() -> None:
    ...
    assert pyfltr.output.rule_urls.build_rule_url("pylint", "missing-module-docstring") is None


def test_build_rule_url_pylint_with_category() -> None:
    """pylint は category 付きで URL を生成する。"""
    url = pyfltr.output.rule_urls.build_rule_url("pylint", "missing-module-docstring", category="convention")
...
def test_build_rule_url_eslint_plugin_unsupported() -> None:
    ...
    assert pyfltr.output.rule_urls.build_rule_url("eslint", "@typescript-eslint/no-explicit-any") is None


def test_build_rule_url_markdownlint() -> None:
    """markdownlint の URL 生成。"""
    url = pyfltr.output.rule_urls.build_rule_url("markdownlint", "MD001")
...
def test_build_rule_url_empty_rule() -> None:
    ...
    assert pyfltr.output.rule_urls.build_rule_url("ruff-check", None) is None
    assert pyfltr.output.rule_urls.build_rule_url("ruff-check", "") is None


def test_build_rule_url_unknown_command() -> None:
    """未登録のコマンド名は None を返す。"""
    assert pyfltr.output.rule_urls.build_rule_url("unknown-tool", "X123") is None
