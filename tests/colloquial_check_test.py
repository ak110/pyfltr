...
class TestScanText:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "一般に有効と言われています。",
            "古くから安全と言われてきた。",
            "計測を終えた。驚くべきことに、誤差は縮小した。",
            "準備は整った。驚くべきことに、失敗は一件も無かった。",
            "計測を終えた。興味深いことに、傾向が反転した。",
            "条件を変えた。興味深いことに、結果は同じだった。",
            "対応は大きく分けて3つある。",
            "手順は大きく分けて二種類ある。",
            "ここからが本題である。",
            "前置きは以上とし、ここからが本題だ。",
    ...
    def test_added_entries_ignore_negative_boundary_samples(
        self, deny_patterns: _PatternList, allow_patterns: _PatternList, text: str
    ) -> None:
        assert not pyfltr.colloquial.check.scan_text(text, deny_patterns, allow_patterns)
