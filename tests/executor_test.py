...
def test_split_commands_estimated_time_order() -> None:
    ...
    _, _, linters_and_testers = pyfltr.state.executor.split_commands_for_execution(commands, config, all_files)
...
def test_split_commands_estimated_time_scales_with_files() -> None:
    ...
    all_files = [pathlib.Path("test.py")]
    _, _, linters = pyfltr.state.executor.split_commands_for_execution(commands, config, all_files)
...
def test_split_commands_include_fix_stage() -> None:
    ...
    fixers, formatters, linters = pyfltr.state.executor.split_commands_for_execution(
...
def test_split_commands_no_fix_stage_by_default() -> None:
    ...
    config = pyfltr.config.config.create_default_config()
    commands = ["ruff-check"]
    fixers, _, _ = pyfltr.state.executor.split_commands_for_execution(commands, config, [])
    assert not fixers
...
def test_serial_group_lock_noop_for_none() -> None:
    ...
    def _worker() -> None:
        with pyfltr.state.executor.serial_group_lock(None):
            started.set()
...
def test_serial_group_lock_mutual_exclusion() -> None:
    ...
    def _worker(index: int) -> int:
        ...
        del index
        with pyfltr.state.executor.serial_group_lock(group):
            with state_lock:
                concurrent_count += 1
...
def test_serial_group_lock_independent_groups() -> None:
    ...
    def _worker_a() -> None:
        with pyfltr.state.executor.serial_group_lock("test-group-a"):
            group_a_running.set()
    ...
    def _worker_b() -> None:
        ...
        with pyfltr.state.executor.serial_group_lock("test-group-b"):
