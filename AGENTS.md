...
- `pyfltr/colloquial/words.txt`の読み込み禁止は、成果物・ドキュメントを書く主体（メインエージェント・
  計画作成担当・レビュー統合主体）のコンテキスト汚染防止を目的とする。これらの主体は編集時に限らず
  調査時も読み込まない。辞書内容の確認は`Explore`サブエージェントへ、編集・検証は辞書専任のwriter・
  reviewer委譲先へ委譲し、委譲先は完了報告・レビュー報告へ検出語を転記しない
  - 運用方針コメントの更新も辞書専任のwriter委譲先へ委譲する。メインエージェント・計画作成担当・
    レビュー統合主体は、対象の行番号の特定と、コメント範囲へ限定した変更後の確認だけを行う
    - 委譲する理由は、実行環境がメインエージェント自身の編集操作とシェル経由の書き換えの双方で
      変更後のファイル全文を当該主体のコンテキストへ提示する場合があり、
      行番号を指定した編集でも辞書エントリー本体の流入を防げないためである
