...
- `.claude/rules/`配下のルールファイルが`paths` frontmatterで連動更新先を列挙する場合、
  ルール改訂・連動先リネーム・新規追加の各タイミングでpathsとリポジトリ実体の整合を計画段階で検証する。
  実在しないパスを残すと条件付きロードのトリガーとして機能しない
- モジュールパス参照を含むドキュメントはモジュール移動の際に追従更新が必要。
  主な対象は`docs/development/architecture.md`・`.claude/rules/`配下
...
- `pyfltr/.claude/rules/grep-replace.md`:
