...
利用者は`{command}-runner = "direct"`または`{command}-path`の明示で個別に戻すことができる。
...
推奨表記はドキュメント側で`uv add --dev "pyfltr[python]"`を採用し、利用者に提示する表記を将来の方針変更に対しても安定させる。
...
利用者ホームディレクトリ依存のパス（例: `~/dotfiles/.../tool.py`）を設定値として記述できるように、
以下のper-toolキーに限り `os.path.expanduser` を適用する。
適用範囲のSSOTは`pyfltr/config/config.py`の`EXPAND_USER_KEY_SUFFIXES`定数。

- `{command}-path`: `pyfltr/command/runner.py`の`build_commandline`で
  path-override経路採用直前に展開する
- `{command}-args`: `build_invocation_argv`で配列要素ごとに展開する
- `{command}-fix-args`: `build_invocation_argv`で配列要素ごとに展開する。
  textlintのfix Step1経路（`--format`ペア除去後）でも同じく展開する
- 上記以外の`{command}-args`（例: `{command}-check-args`・`{command}-write-args`）も、
  両側で挙動を揃えるため`pyfltr/command/two_step/base.py`が
  `pyfltr.command.runner.expanduser_args`を経由して展開する

`config-files`・`targets`・`{command}-extend-targets`等のglobパターンには展開を適用しない。
glob内チルダの意図しない展開を防ぐため対象キーを限定する方針。
展開タイミングはsubprocess引数組み立て直前で、`config.values`読込時点では原文を保持する
（`command-info`サブコマンドの`configured_path`/`configured_args`にも原文が露出する）。

展開は`os.path.expanduser`に委譲するため、各要素の先頭が`~`または`~user`で始まる場合のみ展開される。
`"--config=~/cfg.toml"`のように要素の途中に`~`を含む形式は展開されない。
利用者向けには`["--config", "~/cfg.toml"]`のように分けて指定するよう案内する。
