...
- 日本語文体系（言語非依存）: `targets="*"`で全ファイル種別を対象とし特定言語・特定フォーマットに紐付かないため、
  Markdown系とは別区分として扱う。`colloquial-check`が該当し、`BUILTIN_COMMANDS`登録順は
  `lychee`の直後に配置する（Markdown系末尾への近接配置自体は維持しつつ分類ラベルのみ独立させる）
