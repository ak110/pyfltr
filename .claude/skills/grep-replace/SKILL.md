...
MCPツール`grep`は`max_total`未指定時も全マッチを走査し、`auto_summary=True`を既定として結果部分を10,000文字以内へ縮約する。
コーディングエージェント環境のCLIも自動縮約を既定値として有効にし、通常の端末では全マッチ表示を維持する。
特定ファイル名を判定へ組み込まず、直列化後の削減量が大きい高密度ファイルから件数表示へ切り替える。
低密度ファイルの本文は予算内で維持する。
明示された`max_count`・`max_total`・`max_preview_chars`・集計指定は自動縮約より優先する。
省略されたJSONLを`--from-grep`又はMCPの`from_grep`へ渡す操作は、対象ファイル集合を欠落させるため拒否する。

## 本文プレビューの上限

`grep`が返す本文（`line_text`・`match_text`・前後コンテキストの各行）は、
1件あたり`DEFAULT_MAX_PREVIEW_CHARS`文字までのプレビューとして返す。
既定値はCLIとMCPで分けない。`dry_run`はCLIとMCPで暴発の起こりやすさが異なるため既定値を分けたが、
本文の肥大化は1件の巨大な単一行に一致するだけで起き、CLIとMCPのどちらでも同じ条件で発生するためである。
...
警告は`pyfltr.warnings_`へ1件蓄積し、各形式の既存の警告描画処理で返す。
切り詰め専用の伝達手段は設けない。
