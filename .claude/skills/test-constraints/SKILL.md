...
- 公開値の生成処理を変更する場合、当該値を検証している既存テストを実装側から逆引きで洗い出し、
...
- `scripts/`配下のスクリプトを`tests/`配下から参照する場合は`scripts/__init__.py`を設置する。
  ...
  設置しないと同一ファイルがトップレベル名とパッケージ配下名の双方へ割り当たりチェックが停止する。
  `[tool.pytest.ini_options]`の`pythonpath`は型チェッカーが参照しないため代替にならない
...
- bin-runnerの可用性判定（`ensure_mise_available`が起動する`mise exec <tool spec> -- <bin> --version`）は
  `tests/conftest.py`のautouseフィクスチャ`_default_mise_exec_check_success`が既定値として成功へ固定する。
  ...
  - miseの解決処理の実挙動を検証するテストは`@pytest.mark.real_mise_subprocess`を付けて既定モックを外す。
