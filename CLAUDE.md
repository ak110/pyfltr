...
- `pyfltr/command/`:（フェーズ2で導入予定）コマンド実行コア。現時点では`pyfltr/command.py`がトップレベルに存在する
- `pyfltr/config/`: 設定読み書きとプリセット
  - `config/config.py`: `Config` / `load_config()` / `BUILTIN_COMMANDS`等。設定の読み書き・解決
...
- 命名は責務に沿う。ガイダンス系（`precommit_guidance`等）は`cli`配下、実行系は`command`配下（フェーズ2）
...
- `pyfltr/command.py`: コマンド実行コア（フェーズ2でサブパッケージ化予定）
- `pyfltr/builtin_commands.py`: ビルトインコマンド定義
- `pyfltr/error_parser.py`: エラーパーサー
...
- モジュールパス参照を含むドキュメントはモジュール移動の際に追従更新が必要。
  ...
  サブパッケージ構成変更時（特に`command.py`分割のフェーズ2完了後）はこれらも合わせて更新する
