...
- `pyfltr/command/`: コマンド実行コア
  ...
  - `command/two_step/ruff.py`: `_execute_ruff_format_two_step`
  - `command/two_step/taplo.py`: `_execute_taplo_two_step`
  - `command/two_step/shfmt.py`: `_execute_shfmt_two_step`
  - `command/two_step/prettier.py`: `_execute_prettier_two_step`
  - two-step formatter4種（ruff/taplo/shfmt/prettier）には共通の段階制御パターンが存在し、
    pylintの`duplicate-code`をファイル先頭で抑制している。
    重複ロジックの統合は別計画として切り出す方針で、現状の分割はファイル位置の明確化に留める
